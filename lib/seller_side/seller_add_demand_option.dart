...
class AddFishDemandOptionPage extends StatelessWidget {
  final String sellerId;
  AddFishDemandOptionPage({required this.sellerId});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Add Fish Demand Option'),
      ),
...
class _AddFishOptionFormState extends State<AddFishOptionForm> {
  ...
  Future<void> addFishOption() async {
    ...
    if (fishName.isNotEmpty && photoUrl.isNotEmpty) {
      ...
      await _firestore.collection('seller_info').doc(sellerId).collection('fish_demand_choices').add(newfish_storage.toMap());
